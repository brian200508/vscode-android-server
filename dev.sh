...
main() {
  cd "$(dirname "$0")/"
    ...
    case "$1" in
      ...
      release)
        ...
        if [ ! -z "$BUILD_NODE" ]; then
          ...
          $USERRUN PATH=/vscode-build/hostbin:$PATH CC_host=gcc CXX_host=g++ LINK_host=g++ ./android-configure /usr/lib/android-sdk/ndk/ $ANDROID_BUILD_API_VERSION $NODE_CONFIGURE_NAME
	...
	export VERSION=$(cd code-server && git describe --tags)
        ...
        cp /usr/lib/android-sdk/ndk/sources/cxx-stl/llvm-libc++/libs/$ARCH_NAME/libc++_shared.so ./libc++_shared.so
        VERSION_SUFFIX=
